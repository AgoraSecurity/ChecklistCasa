...
class EmailService:
    ...
    def _validate_settings(self) -> None:
        ...
        if not self.api_key:
            logger.warning(
                "MAILGUN_API_KEY is not set - email functionality will be disabled"
            )
            return
        if not self.domain:
            logger.warning(
                "MAILGUN_DOMAIN is not set - email functionality will be disabled"
            )
            return
        if not self.default_from_email:
            logger.warning(
                "DEFAULT_FROM_EMAIL is not set - email functionality will be disabled"
            )
            return

    def _send_email(
        self, subject: str, html_content: str, recipient_email: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not all([self.api_key, self.domain, self.default_from_email]):
                ...
                return {"status": "skipped", "message": "Email settings not configured"}
    ...
    def send_invitation_email(
        self, email: str, project_name: str, invitation_url: str, invited_by_email: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return self._send_email(
                subject=subject, html_content=html_content, recipient_email=email
            )
    ...
    def send_home_upload_confirmation(self, user, home_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                or not user.profile.receive_confirmation_emails
            ):
                logger.info(
                    f"Skipping confirmation email for user {user.email} - opted out"
                )
                return {
                    "status": "skipped",
            ...
            return self._send_email(
                subject=subject, html_content=html_content, recipient_email=user.email
            )
